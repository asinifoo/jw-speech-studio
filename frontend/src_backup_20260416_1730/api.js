...
export async function outlineDetail(outlineId, outlineType = '') {
  const params = outlineType ? `?outline_type=${encodeURIComponent(outlineType)}` : '';
  const res = await fetch(`${API}/outline/${encodeURIComponent(outlineId)}${params}`);
  return res.json();
}
