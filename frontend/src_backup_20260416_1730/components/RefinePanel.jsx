  ...
  return (
    <div style={{ borderRadius: 10, border: '1px solid var(--bd)', background: 'var(--bg-card)', overflow: 'hidden', marginBottom: 14 }}>
      ...
      <div style={{ padding: '10px 14px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--bd)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <span style={{ fontSize: '1.0rem', fontWeight: 700, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 4, position: 'relative' }}>
          {!isActive && !refineMode ? (
            ...
            <button onClick={() => setRefineMode(false)}
              style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-faint)', fontSize: '0.786rem', cursor: 'pointer' }}>취소</button>
          ) : isActive ? (
            <button onClick={() => { abortRef.current?.abort(); abortGeneration(); }}
              style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid #e55', background: '#e5511a', color: '#fff', fontSize: '0.786rem', cursor: 'pointer', fontWeight: 600, position: 'relative' }}>중단</button>
          ) : null}
        </div>
      </div>
      ...
      {/* Error */}
      {error && (
        <div style={{ padding: '8px 14px', background: '#c4441a', color: '#fff', fontSize: '0.857rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{error}</span>
          <button onClick={onClearError} style={{ border: 'none', background: 'none', color: '#fff', fontSize: '1.143rem', cursor: 'pointer', padding: '0 4px', lineHeight: 1, flexShrink: 0 }}>×</button>
        </div>
      )}
      ...
      {refineMode ? (
        <div style={{ padding: 14 }}>
          ...
          <div style={{ marginTop: 10, borderRadius: 8, border: '1px solid var(--opt-bd)', background: 'var(--opt-bg)', padding: 12 }}>
            ...
            <div style={{ fontSize: '0.786rem', color: 'var(--c-muted)', marginBottom: 2 }}>추가 지시사항</div>
        ...
        <div style={{ padding: '16px 20px', fontSize: '0.857rem', lineHeight: 2, whiteSpace: 'pre-wrap', maxHeight: 500, overflowY: 'auto' }}>
          {script}
        </div>
      )}
    </div>
