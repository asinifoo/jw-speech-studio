...
export default function AiModelSelector({ ai, showGenCtx = true }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '0.786rem', color: 'var(--c-muted)', marginBottom: 3 }}>AI 모델</div>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        ...
        <button onClick={isDefaultModel ? clearDefault : saveDefault}
          ...
          style={{
            ...
            border: `1.5px solid ${isDefaultModel ? 'var(--accent-orange)' : 'var(--bd)'}`,
            background: isDefaultModel ? '#D85A3018' : 'transparent',
            color: isDefaultModel ? 'var(--accent-orange)' : 'var(--c-sub)',
      ...
      {llmSettings && showGenCtx && aiPlatform === 'Local' && (
        <div style={{ padding: '4px 8px', borderRadius: 8, background: 'var(--bg-subtle)', marginTop: 6, fontSize: '0.786rem', color: 'var(--c-muted)', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <span>생성 CTX: <b style={{ color: 'var(--accent-orange)' }}>{(llmSettings.gen_ctx / 1024).toFixed(0)}K</b></span>
          <span>·</span>
          <span>생성 🧠 <b style={{ color: llmSettings.gen_no_think ? 'var(--c-muted)' : 'var(--accent-purple)' }}>{llmSettings.gen_no_think ? 'OFF' : 'ON'}</b></span>
          <span style={{ fontSize: '0.643rem', color: 'var(--c-dim)' }}>Manage에서 변경</span>
        </div>
      )}
    </div>
  );
