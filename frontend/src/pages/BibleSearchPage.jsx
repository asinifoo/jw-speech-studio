  ...
  return (
    <div>
      <div style={{ borderRadius: 12, border: '1px solid var(--bd)', background: 'var(--bg-card)', marginBottom: 16, overflow: 'hidden' }}>
        <div style={{ padding: '12px 14px 8px' }}>
          <KoreanTextarea
            ...
            style={{
              ...
              border: 'none', borderRadius: 8, fontSize: '0.929rem', outline: 'none',
        ...
        <div style={{
          ...
          {query.trim().split('\n').filter(l => l.trim()).length > 1 && (
            <span style={{ fontSize: '0.786rem', color: 'var(--c-dim)' }}>여러 줄 감지</span>
          )}
          <div style={{ flex: 1 }} />
      ...
      {results.length === 0 && !loading && query && (
        <div style={{ textAlign: 'center', padding: 30, color: 'var(--c-dim)', fontSize: '0.929rem' }}>검색 결과가 없습니다</div>
      )}
    </div>
  );
}
