...
export default function ServiceMeetingPage({ fontSize, ai }) {
  ...
  const [pastEditIdx, setPastEditIdx] = useState(null); // 임시 수정 중인 인덱스
  const [pastEditText, setPastEditText] = useState({}); // { idx: text } 임시 수정 텍스트
  const [pastDbIdx, setPastDbIdx] = useState(null); // DB 편집 중인 인덱스
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      setPastMeetings((pastRes.entries || []).sort((a, b) => {
                    ...
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      <button onClick={async () => {
                        ...
                        try {
                          ...
                          setPastMeetings(p => p.map((x, j) => j === i ? { ...x, text: pastDbText, metadata: saveMeta } : x));
