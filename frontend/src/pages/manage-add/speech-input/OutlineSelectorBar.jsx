    ...
    <div style={{ marginBottom: 10 }}>
      ...
      {!noOutline && (
        <div style={{ position: 'relative' }}>
          ...
          {queryFocus && query.trim() && (() => {
            ...
            const matched = outlines.filter(g => (g.outline_num || '').toLowerCase().includes(q) || (g.outline_title || '').toLowerCase().includes(q) || (g.outline_type_name || '').toLowerCase().includes(q));
      ...
      {outline && !noOutline && (
        <div style={{ marginTop: 6, padding: '6px 10px', borderRadius: 8, background: 'var(--tint-green)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: '0.786rem', color: 'var(--c-text)' }}>{outline.outline_title}</span>
          <div style={{ flex: 1 }} />
