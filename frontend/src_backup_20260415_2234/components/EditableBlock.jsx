      ...
      <div style={{ borderRadius: 8, border: '1px solid ' + borderColor, background: bgColor, overflow: 'hidden' }}>
        <div style={{
          ...
          <button onClick={() => { if (value) setEditing(false); }} style={{
            ...
            background: value ? color : 'var(--bd)', color: '#fff', fontSize: 10, cursor: 'pointer', fontWeight: 600,
          }}>확인</button>
          <button onClick={() => { onChange(''); setEditing(false); }} style={{
            padding: '2px 8px', borderRadius: 4, border: '1px solid ' + borderColor,
            background: 'var(--bg-card)', color: 'var(--c-muted)', fontSize: 10, cursor: 'pointer',
          }}>취소</button>
        </div>
        <div style={{ padding: 8 }}>
          <KoreanTextarea
  ...
  return (
    <div style={{ borderRadius: 8, border: '1px solid ' + borderColor, background: bgColor, overflow: 'hidden' }}>
      <div style={{
        ...
        <button onClick={() => setEditing(true)} style={{
          padding: '2px 8px', borderRadius: 4, border: '1px solid ' + borderColor,
          background: 'var(--bg-card)', color: color, fontSize: 10, cursor: 'pointer',
        }}>수정</button>
      ...
      <div style={{ padding: '8px 10px' }}>
        <div style={{
          fontSize: 13, lineHeight: 1.8, color: 'var(--c-text)',
        ...
        {value.length > 200 && (
          <button onClick={() => setExpanded(!expanded)} style={{
            ...
            color: color, fontSize: 11, cursor: 'pointer',
