  ...
  if (compact && !open) {
    return (
      <button onClick={() => setOpen(true)} style={{
        ...
        background: 'transparent', color: 'var(--c-dim)', fontSize: 10, cursor: 'pointer',
      }}>▾ 불용어</button>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--c-text-dark)', flex: 1 }}>검색 키워드 필터</span>
        <span style={{
          fontSize: 9, color: 'var(--c-dim)', background: 'var(--bg-subtle)',
          padding: '2px 6px', borderRadius: 4,
        }}>접미사 {sfxCount} · 불용어 {swCount}</span>
        {compact && (
          <button onClick={() => setOpen(false)} style={{
            width: 20, height: 20, borderRadius: 4, border: '1px solid var(--bd)',
            background: 'transparent', color: 'var(--c-dim)', fontSize: 11,
      ...
      <div style={{ padding: 12 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: 10, color: 'var(--c-dim)', flexShrink: 0 }}>테스트</span>
            <input value={testInput} onChange={e => { setTestInput(e.target.value); setTestResult(''); }}
              ...
              style={{
                ...
                background: 'var(--bg-input)', color: 'var(--c-text-dark)', fontSize: 12,
                fontFamily: 'inherit', outline: 'none',
              }} />
          </div>
          {testResult && (
            <div style={{
              fontSize: 11, fontWeight: 700, color: '#C7842D',
          ...
          {[
            ...
            <div key={c.label} style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 5 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--c-text-dark)' }}>{c.label}</span>
                <span style={{ fontSize: 8, color: 'var(--c-dim)' }}>{c.desc}</span>
              </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={doSave} style={{
            padding: '5px 14px', borderRadius: 8, border: 'none', fontSize: 10, fontWeight: 700,
            background: '#C7842D', color: '#fff', cursor: 'pointer',
          }}>저장</button>
          <button onClick={doSaveDefault} style={{
            padding: '5px 14px', borderRadius: 8, border: '1px solid #2D8FC7', fontSize: 10, fontWeight: 600,
            background: 'transparent', color: '#2D8FC7', cursor: 'pointer',
          }}>기본값으로 저장</button>
          <button onClick={doReset} style={{
            padding: '5px 14px', borderRadius: 8, border: '1px solid var(--bd)', fontSize: 10,
