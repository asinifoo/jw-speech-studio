    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 400 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div onClick={() => setShowModelSelector(p => !p)}
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: '#7F77DD', flexShrink: 0 }}>{ai.chatPlatform}</span>
            <span style={{ fontSize: 12, color: 'var(--c-text-dark)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{chatModelLabel}</span>
            {ai.isChatDefaultModel && <span style={{ fontSize: 9, color: '#7F77DD', flexShrink: 0 }}>★</span>}
            <span style={{ fontSize: 8, color: 'var(--c-dim)', flexShrink: 0 }}>{showModelSelector ? '▲' : '▼'}</span>
          </div>
          <button onClick={() => setShowSessionList(p => !p)} style={{
            ...
            fontSize: 9, cursor: 'pointer', fontWeight: showSessionList ? 600 : 400, flexShrink: 0,
          }}>목록{sessions.length > 0 ? ` ${sessions.length}` : ''}</button>
          <button onClick={newChat} style={{
            padding: '3px 8px', borderRadius: 8, border: '1px solid var(--bd)',
            background: 'transparent', color: 'var(--c-muted)', fontSize: 9, cursor: 'pointer', flexShrink: 0,
          }}>+ 새 대화</button>
        </div>
        {/* 2줄: 설정 뱃지 */}
        {ai?.llmSettings && (
          <div style={{ display: 'flex', gap: 3, fontSize: 8, alignItems: 'center', marginTop: 5, flexWrap: 'wrap' }}>
          ...
          {needsPassword && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10 }}>
              ...
              <input type={showPw ? 'text' : 'password'} autoComplete="off" value={password} onChange={e => setPassword(e.target.value)}
                ...
                style={{ flex: 1, padding: '4px 8px', borderRadius: 8, border: '1px solid ' + (password ? '#1D9E75' : 'var(--bd)'),
                  background: 'var(--bg-input)', color: 'var(--c-text-dark)', fontSize: 11, fontFamily: 'inherit', outline: 'none' }} />
              <button onClick={() => setShowPw(p => !p)}
                style={{ padding: '2px 6px', borderRadius: 4, border: '1px solid var(--bd)', background: 'transparent',
                  color: 'var(--c-dim)', fontSize: 9, cursor: 'pointer' }}>{showPw ? '숨김' : '표시'}</button>
            </div>
          )}
        </div>
      )}
      ...
      {showSessionList && (
        <div style={{ marginBottom: 8, borderRadius: 8, border: '1px solid var(--bd)', background: 'var(--bg-card)', maxHeight: 200, overflowY: 'auto' }} className="chat-input">
          {sessions.length === 0 && (
            <div style={{ padding: 12, textAlign: 'center', fontSize: 11, color: 'var(--c-dim)' }}>저장된 대화가 없습니다</div>
          )}
          {sessions.map(s => (
            <div key={s.id} onClick={() => loadSession(s.id)} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: currentId === s.id ? 700 : 400, color: currentId === s.id ? '#7F77DD' : 'var(--c-text-dark)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {s.title || '새 대화'}
                </div>
              ...
              <button onClick={(e) => removeSession(s.id, e)} style={{
                ...
                background: 'transparent', color: 'var(--c-dim)', fontSize: 9, cursor: 'pointer',
              }}>삭제</button>
            </div>
          ))}
        </div>
      ...
      <div style={{ flex: 1, marginBottom: 8 }}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 16px', color: 'var(--c-dim)' }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--c-muted)', marginBottom: 6 }}>
            ...
            {m.role === 'user' && (
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{
                  ...
                  background: '#1D9E75', color: '#fff', fontSize: 13, lineHeight: 1.7,
            ...
            {m.role === 'assistant' && (
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0, maxWidth: '85%' }}>
                  {m.loading && !m.content && (
                    <div style={{ padding: '10px 0', fontSize: 12, color: 'var(--c-muted)' }}>
                  ...
                  {m.content && (
                    <div style={{
                      ...
                      fontSize: 13, lineHeight: 1.7, color: m.error ? '#c44' : 'var(--c-text)',
            ...
            return (
              <div key={rsi} style={{ marginBottom: 6 }}>
                <button onClick={() => setExpandedResultIdx(isOpen ? -1 : rsi)} style={{
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#fff', background: modeColor, width: 20, height: 20, borderRadius: 8, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{rsi + 1}</span>
                  <span style={{ flex: 1, color: isOpen ? 'var(--c-text-dark)' : 'var(--c-muted)', fontWeight: isOpen ? 600 : 400 }}>{queryShort}</span>
                  {rs.mode && rs.mode !== 'db' && <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 8, fontWeight: 600, background: modeColor + '18', color: modeColor }}>{rs.mode === 'wol' ? 'WOL' : 'DB+WOL'}</span>}
                  <span style={{ fontSize: 10, color: modeColor, fontWeight: 600, padding: '2px 6px', borderRadius: 8, background: modeColor + '10' }}>{rs.results.length}건</span>
                  <span style={{ fontSize: 10, color: 'var(--c-dim)', transition: 'transform 0.2s', transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>▶</span>
                </button>
                {isOpen && (
                        ...
                        <div key={ri} onClick={() => setExpandedCards(p => ({ ...p, [cardKey]: !p[cardKey] }))} style={{ borderRadius: 8, overflow: 'hidden', marginBottom: 8, cursor: 'pointer', border: '1px solid var(--bd-soft)', background: 'var(--bg-card)' }}>
                          <div style={{ padding: '8px 10px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--bd-light)' }}>
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                              ...
                              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--c-hint)' }}>{sourceLabel[meta.source] || colLabel[col] || col}</span>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4 }}>
                              <div style={{ flex: 1 }} />
                              {meta.wol_url && <a href={meta.wol_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ padding: '2px 8px', borderRadius: 4, fontSize: 9, fontWeight: 600, background: '#C7842D', color: '#fff', textDecoration: 'none' }}>WOL ↗</a>}
                              <button onClick={e => { e.stopPropagation(); doCopy('card-' + cardKey, body); }} style={{ padding: '2px 6px', borderRadius: 4, border: '1px solid ' + (copied['card-' + cardKey] ? '#1D9E75' : 'var(--bd)'), background: copied['card-' + cardKey] ? 'var(--tint-green)' : 'var(--bg-card)', color: copied['card-' + cardKey] ? '#1D9E75' : 'var(--c-faint)', fontSize: 9, cursor: 'pointer', fontWeight: 600 }}>{copied['card-' + cardKey] ? '✓ 복사됨' : '복사'}</button>
                            </div>
                          </div>
                          {metaRows.length > 0 && (
                            <div style={{ padding: '8px 10px', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 8px', alignItems: 'baseline', fontSize: 12, lineHeight: 1.8, color: 'var(--c-sub)' }}>
                              {metaRows.map((row, mi) => (<Fragment key={mi}><span style={{ fontSize: 9, color: 'var(--c-dim)', whiteSpace: 'nowrap' }}>{row.label}</span><span style={{ fontSize: 11, color: row.color || 'var(--c-text)', lineHeight: 1.5, wordBreak: 'keep-all' }}>{row.value}</span></Fragment>))}
                            </div>
                          )}
                          {body && (
                            <div style={{ padding: '6px 10px 10px', borderTop: '1px solid var(--bd-light)' }}>
                              <div style={{ fontSize: 13, lineHeight: 1.9, color: 'var(--c-text)', whiteSpace: 'pre-wrap', wordBreak: 'keep-all', maxHeight: isExp ? (col === 'wol' ? 400 : 250) : 80, overflow: isExp ? 'auto' : 'hidden' }}>
                                {body.length > 150 && !isExp ? body.slice(0, 150) + '...' : body}
                              </div>
                              {body.length > 150 && <button onClick={e => { e.stopPropagation(); setExpandedCards(p => ({ ...p, [cardKey]: !p[cardKey] })); }} style={{ marginTop: 4, padding: '2px 10px', borderRadius: 4, border: '1px solid var(--bd)', background: 'var(--bg-subtle)', color: 'var(--c-faint)', fontSize: 10, cursor: 'pointer' }}>{isExp ? '접기' : '전체 보기'}</button>}
                            </div>
                          )}
                        </div>
                      );
      ...
      <div style={{
        ...
        {attachedFile && (
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#7F77DD', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {attachedFile.name}
              </div>
              <div style={{ fontSize: 9, color: 'var(--c-dim)', marginTop: 1 }}>
            ...
            <button onClick={() => setAttachedFile(null)} style={{
              ...
              background: '#7F77DD20', color: '#7F77DD', fontSize: 10,
              cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>✕</button>
          </div>
        )}
        {fileUploading && (
          <div style={{ margin: '8px 12px 0', fontSize: 10, color: '#7F77DD' }}>
            파일 읽는 중...
          </div>
        )}
            ...
            {[
              ...
              return (
                <button key={m.key} onClick={() => !m.disabled && setSearchMode(m.key)}
                  disabled={m.disabled}
                  style={{
                    padding: '5px 12px', borderRadius: 8, fontSize: 11.5, fontWeight: active ? 700 : 500,
                    border: 'none',
        ...
        <div style={{
          ...
          {wolAvailable && (
            <button onClick={() => setShowWolFilters(p => !p)} style={{
              ...
              fontSize: 18, cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0,
            }}>▾</button>
          )}
          <div style={{ flex: 1 }} />
          <button onClick={() => fileRef.current?.click()} disabled={fileUploading || streaming}
            ...
            style={{
              ...
              fontSize: 18, fontWeight: 300, cursor: 'pointer', flexShrink: 0,
          ...
          {streaming ? (
            <button onClick={() => { abortRef.current?.abort(); abortGeneration(); }} style={{
              ...
              background: '#e55', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', flexShrink: 0,
              textAlign: 'center',
            }}>■ 중지</button>
          ) : (
