    ...
    <div>
      ...
      <div style={{ borderRadius: 12, border: '1px solid var(--bd)', background: 'var(--bg-card)', marginBottom: 14, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 14px 8px' }}>
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 10, color: 'var(--c-muted)', marginBottom: 4 }}>봉사 종류</div>
            <div style={{
              ...
              {serviceTypes.map(t => (
                <button key={t} onClick={() => toggleType(t)} style={{
                  padding: '5px 12px', borderRadius: 8, fontSize: 11.5, fontWeight: selTypes.has(t) ? 700 : 500,
                  border: 'none',
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <input value={scriptures} onChange={e => setScriptures(e.target.value)} placeholder="핵심 성구: 막 4:14, 15; 마 5:37"
              style={{ flex: 1, minWidth: 0, padding: '8px 12px', border: 'none', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', outline: 'none', color: 'var(--c-text-dark)', background: 'var(--bg-subtle)', boxSizing: 'border-box' }} />
            <input value={duration} onChange={e => setDuration(e.target.value)} placeholder="시간"
              style={{ width: 70, flexShrink: 0, padding: '8px 10px', border: 'none', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', outline: 'none', color: 'var(--c-text-dark)', background: 'var(--bg-subtle)', boxSizing: 'border-box', textAlign: 'center' }} />
          </div>
          <textarea value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="추가 검색어 (선택)" rows={3}
            style={{ width: '100%', padding: '8px 12px', border: 'none', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', outline: 'none', color: 'var(--c-text-dark)', background: 'var(--bg-subtle)', boxSizing: 'border-box', resize: 'vertical', lineHeight: 1.7 }} />
        </div>
        {/* 하단 바 */}
        <div style={{
          ...
          <button onClick={() => { const next = !useLLMFilter; setUseLLMFilter(next); if (!next) setShowFilters(false); }} style={{
            ...
            {useLLMFilter && <span onClick={e => { e.stopPropagation(); setShowFilters(p => !p); }}
              style={{ color: showFilters ? '#C7842D' : '#7F77DD80', fontSize: 18, lineHeight: 0 }}>▾</span>}
          </button>
          <div style={{ flex: 1 }} />
          ...
          <button onClick={doSearch} disabled={searchLoading || (!selTypes.size && !scriptures.trim() && !searchQuery.trim())} style={{
            ...
            fontSize: 11, fontWeight: 700, cursor: (searchLoading || (!selTypes.size && !scriptures.trim() && !searchQuery.trim())) ? 'default' : 'pointer',
      ...
      {autoScriptures.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>성구</div>
          {autoScriptures.map((a, i) => (
            <div key={i} style={{ padding: '8px 10px', borderRadius: 7, background: 'var(--tint-blue)', border: '1px solid var(--tint-blue-bd)', marginBottom: 6 }}>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 3 }}>
                <span style={{ width: 16, height: 16, borderRadius: 3, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 800, color: '#fff', background: '#D85A30' }}>B</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#2a7ab5' }}>{a.original || a.book}</span>
              </div>
              <div style={{ paddingLeft: 22, fontSize: 13, lineHeight: 1.7 }}>
                {(a.verses || []).map((v, vi) => (
      ...
      {/* 과거 봉사 모임 */}
      {phase >= 1 && pastLoading && <div style={{ textAlign: 'center', color: 'var(--c-muted)', fontSize: 12, marginBottom: 14 }}>과거 봉사 모임 검색 중...</div>}
      {phase >= 1 && !pastLoading && pastMeetings.length === 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>과거 봉사 모임 (0건)</div>
          <div style={{ fontSize: 11, color: 'var(--c-dim)', padding: 8 }}>저장된 봉사 모임이 없습니다.</div>
        </div>
      )}
      {phase >= 1 && !pastLoading && pastMeetings.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>
            ...
            return (
              <div key={i} onClick={() => setSelectedPast(p => ({ ...p, [i]: !p[i] }))}
                ...
                <div style={{ padding: '6px 10px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', background: sel ? 'var(--tint-green)' : 'var(--bg-subtle)', borderBottom: '1px solid var(--bd-light)' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--c-hint)', fontWeight: 600 }}>{meta.date || '?'}</span>
                  {meta.service_type && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'var(--tint-green-soft)', color: '#2e7d32', fontWeight: 600 }}>{meta.service_type}</span>}
                  <span style={{ fontSize: 11, color: 'var(--c-faint)' }}>{meta.outline_title || meta.topic || ''}</span>
                </div>
                <div style={{ padding: '8px 10px', fontSize: 11, color: 'var(--c-faint)' }}>
                  {parsed?.scripture && <span style={{ marginRight: 8 }}>성구: {parsed.scripture}</span>}
                  {parsed?.keywords && parsed.keywords.split(',').map((kw, ki) => (
                    <span key={ki} style={{ display: 'inline-block', fontSize: 10, padding: '0 5px', borderRadius: 3, background: 'var(--bg-muted)', color: '#777', marginRight: 3, marginBottom: 2 }}>{kw.trim()}</span>
                  ))}
                </div>
                <div style={{ padding: '6px 10px 10px', fontSize: 13, lineHeight: 1.8, color: 'var(--c-sub)', maxHeight: expandedPast[i] ? 400 : 80, overflow: expandedPast[i] ? 'auto' : 'hidden', whiteSpace: 'pre-wrap', wordBreak: 'keep-all' }}>
                ...
                {body.length > 150 && (
                  <div style={{ padding: '2px 10px 6px' }}>
                    <button onClick={(e) => { e.stopPropagation(); setExpandedPast(p => ({ ...p, [i]: !p[i] })); }} style={{
                      padding: '2px 10px', borderRadius: 4, border: '1px solid var(--bd)', background: 'var(--bg-subtle)', color: 'var(--c-faint)', fontSize: 10, cursor: 'pointer',
          ...
          {!showAllPast && pastMeetings.length > 10 && (
            <button onClick={() => setShowAllPast(true)} style={{ width: '100%', padding: '8px 0', borderRadius: 8, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-sub)', fontSize: 11, cursor: 'pointer', marginTop: 6 }}>
      ...
      {phase >= 1 && searchResults.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>DB 검색 결과 ({searchResults.length}건)</div>
            ...
            return (
              <div key={i} onClick={() => setSelectedSearch(p => ({ ...p, [i]: p[i] === false ? true : false }))}
                ...
                <div style={{ padding: '8px 10px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', background: r.filtered ? 'var(--tint-red)' : 'var(--bg-subtle)', borderBottom: '1px solid var(--bd-light)' }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--c-hint)' }}>{sourceLabel[meta.source] || meta.source || (col === 'speech_expressions' ? '연설' : '출판물')}</span>
                  {!isPub && meta.speaker && <span style={{ fontSize: 11, color: 'var(--c-faint)' }}>{meta.speaker}</span>}
                  ...
                  {r.filtered && <span style={{ fontSize: 10, fontWeight: 700, color: '#c44' }}>LLM 제외</span>}
                  {meta.tags && (() => {
                    const t = meta.tags;
                    const badges = [];
                ...
                <div style={{ padding: '6px 10px 10px', fontSize: 13, lineHeight: 1.8, color: 'var(--c-sub)', maxHeight: expandedSearch[i] ? 300 : 80, overflow: expandedSearch[i] ? 'auto' : 'hidden', whiteSpace: 'pre-wrap', wordBreak: 'keep-all' }}>
          ...
          {!showAllResults && searchResults.length > 20 && (
            <button onClick={() => setShowAllResults(true)} style={{ width: '100%', padding: '8px 0', borderRadius: 8, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-sub)', fontSize: 11, cursor: 'pointer', marginTop: 6 }}>
