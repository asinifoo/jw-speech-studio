...
export default function VisitPage({ fontSize, ai }) {
  ...
  return (
    <div>
      <div style={{ borderRadius: 12, border: '1px solid var(--bd)', background: 'var(--bg-card)', marginBottom: 14, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 14px 8px' }}>
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 10, color: 'var(--c-muted)', marginBottom: 4 }}>연령대</div>
            <div style={{
              ...
              {['청소년', '청년', '중년', '장년'].map(s => (
                <button key={s} onClick={() => setAgeGroup(ageGroup === s ? '' : s)} style={{
                  flex: 1, padding: '5px 0', borderRadius: 8, fontSize: 11.5, fontWeight: ageGroup === s ? 700 : 500,
                  border: 'none',
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 10, color: 'var(--c-muted)', marginBottom: 4 }}>고려한 상황</div>
            <div style={{
              ...
              {situations.map(t => (
                <button key={t} onClick={() => toggleSit(t)} style={{
                  padding: '5px 12px', borderRadius: 8, fontSize: 11.5, fontWeight: selSits.has(t) ? 700 : 500,
                  border: 'none',
        ...
        <div style={{
          ...
          <button onClick={doSearch} disabled={searchLoading || !hasInput} style={{
            ...
            fontSize: 11, fontWeight: 700, cursor: searchLoading || !hasInput ? 'default' : 'pointer',
      ...
      {/* 과거 방문 */}
      {phase >= 1 && pastLoading && <div style={{ textAlign: 'center', color: 'var(--c-muted)', fontSize: 12, marginBottom: 14 }}>과거 방문 검색 중...</div>}
      {phase >= 1 && !pastLoading && pastVisits.length === 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>과거 방문 (0건)</div>
          <div style={{ fontSize: 11, color: 'var(--c-dim)', padding: 8 }}>저장된 방문이 없습니다. Manage {'>'} Add에서 추가하세요.</div>
        </div>
      )}
      {phase >= 1 && !pastLoading && pastVisits.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-hint)', marginBottom: 6 }}>과거 방문 ({pastVisits.length}건)</div>
            ...
            return (
              <div key={i} onClick={() => setSelectedPast(p => ({ ...p, [i]: !p[i] }))}
                ...
                <div style={{ padding: '6px 10px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', background: sel ? 'var(--tint-green)' : 'var(--bg-subtle)', borderBottom: '1px solid var(--bd-light)' }}>
                  ...
                  {meta.sub_source && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'var(--tint-orange-soft)', color: '#D85A30', fontWeight: 600 }}>{meta.sub_source}</span>}
                  {sits.map((s, si) => s && s !== '일반' && <span key={si} style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'var(--tint-blue-light)', color: '#378ADD', fontWeight: 600 }}>{s}</span>)}
            ...
            return (
              <div key={i} onClick={() => setSelectedSearch(p => ({ ...p, [i]: p[i] === false ? true : false }))}
                ...
                <div style={{ padding: '8px 10px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', background: r.filtered ? 'var(--tint-red)' : 'var(--bg-subtle)', borderBottom: '1px solid var(--bd-light)' }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--c-hint)' }}>{sourceLabel[meta.source] || meta.source || '연설'}</span>
