...
import { parseOutline, searchPoints, filterResults, generateSpeechStream, healthCheck, abortGeneration, getMyStyles, saveMyStyles, searchSpeakerMemo, getPrompts } from './api';
...
export default function App() {
  ...
  useEffect(() => {
    const onScroll = () => setShowScrollDown(document.documentElement.scrollHeight - window.scrollY - window.innerHeight > 400);
  ...
  useEffect(() => { try { localStorage.setItem('jw-dark', darkMode ? '1' : '0'); } catch(e) {} document.documentElement.classList.toggle('dk', darkMode); const bg = darkMode ? '#1a1a1a' : '#f5f5f5'; document.body.style.background = bg; document.documentElement.style.background = bg; }, [darkMode]);
  const dk = darkMode;
  ...
  const taStyle = {
    ...
    border: '2px solid var(--bd)', borderRadius: 8, background: 'var(--bg-input)', color: 'var(--c-text-dark)',
    fontSize: 13, lineHeight: 1.8, fontFamily: 'inherit', outline: 'none', resize: 'vertical',
  };
...
input, select, textarea {
  background: var(--bg-input); color: var(--c-text-dark); border-color: var(--bd);
}
...
`}</style>
    <div className={`jw-app${dk ? ' dk' : ''}`} style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px 60px', fontFamily: "'Noto Sans KR', -apple-system, sans-serif", color: 'var(--c-text)', zoom: fontSize / 14, background: 'var(--bg)', minHeight: '100vh' }}>

      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ fontSize: 18, fontWeight: 800 }}>JW Speech Studio</div>
          <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showFontSlider && (
              <div style={{ position: 'absolute', top: 36, right: 0, background: 'var(--bg-card)', border: '1px solid var(--bd)', borderRadius: 10, padding: '10px 14px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 20, display: 'flex', alignItems: 'center', gap: 8, minWidth: 180 }}>
                <span onClick={() => setFontSize(f => Math.max(12, f - 1))} style={{ fontSize: 11, color: 'var(--c-muted)', cursor: 'pointer', userSelect: 'none' }}>A</span>
                ...
                <span onClick={() => setFontSize(f => Math.min(20, f + 1))} style={{ fontSize: 15, color: 'var(--c-muted)', fontWeight: 700, cursor: 'pointer', userSelect: 'none' }}>A</span>
                <span style={{ fontSize: 11, color: 'var(--c-muted)', minWidth: 18, textAlign: 'center' }}>{fontSize}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ borderRadius: 10, border: '1px solid var(--bd-soft)', background: 'var(--bg-input)', padding: '10px 12px', marginBottom: 16 }}>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 2,
        background: 'var(--bg-subtle, #f5f5f5)', borderRadius: 10, padding: 2,
      }}>
        {[['speech', '준비'], ['search', '검색'], ['add', '추가'], ['manage', '관리']].map(([key, label]) => (
          <button key={key} onClick={() => setPage(key)} style={{
            flex: 1, padding: '6px 0', border: 'none', fontSize: 12, fontWeight: page === key ? 700 : 500, cursor: 'pointer',
        ...
        <div style={{
          ...
          {[['chat', 'AI 대화'], ['free', 'DB 검색'], ['bible', '성구'], ['original', '원문']].map(([k, l]) => (
            <button key={k} onClick={() => setSearchMode(k)} style={{
              flex: 1, padding: '7px 0', border: 'none', fontSize: 12, fontWeight: searchMode === k ? 700 : 500, cursor: 'pointer',
      ...
      <div style={{
        ...
        {[['speech', '연설'], ['service', '봉사 모임'], ['visit', '방문']].map(([k, l]) => (
          <button key={k} onClick={() => { setPrepareMode(k); setError(''); }} style={{
            flex: 1, padding: '7px 0', border: 'none', fontSize: 12, fontWeight: prepareMode === k ? 700 : 500, cursor: 'pointer',
      ...
      <div style={{ borderRadius: 12, border: '1px solid var(--bd)', background: 'var(--bg-card)', marginBottom: 16, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 14px 8px' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <input type="text" value={speechTitle} onChange={e => setSpeechTitle(e.target.value)} placeholder="연설 제목 (선택)"
              style={{ flex: 1, minWidth: 120, padding: '8px 12px', border: 'none', borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: 'inherit', color: 'var(--c-text-dark)', background: 'var(--bg-subtle)', boxSizing: 'border-box' }} />
            <input type="text" value={speechDuration} onChange={e => setSpeechDuration(e.target.value)} placeholder="시간"
              style={{ width: 70, flexShrink: 0, padding: '8px 10px', border: 'none', borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: 'inherit', color: 'var(--c-text-dark)', background: 'var(--bg-subtle)', textAlign: 'center', boxSizing: 'border-box' }} />
          </div>
        ...
        <div style={{
          ...
          <button onClick={() => { const next = !useLLMFilter; setUseLLMFilter(next); if (!next) setShowPrepareFilters(false); }} style={{
            ...
            background: useLLMFilter ? '#7F77DD15' : 'var(--bg-subtle, #f5f5f5)',
            ...
            {useLLMFilter && <span onClick={e => { e.stopPropagation(); setShowPrepareFilters(p => !p); }}
              style={{ color: showPrepareFilters ? '#C7842D' : '#7F77DD80', fontSize: 18, lineHeight: 0 }}>▾</span>}
          </button>
          <button onClick={() => setSearchTitle(p => !p)} style={{
            padding: '4px 10px', borderRadius: 8, border: 'none',
            background: searchTitle ? '#D85A3015' : 'var(--bg-subtle, #f5f5f5)',
          ...
          <button onClick={run} disabled={!input.trim() || !!status}
            style={{
              ...
              fontSize: 11, fontWeight: 700, background: input.trim() && !status ? '#1D9E75' : 'var(--bd-medium)', color: '#fff',
        ...
        {useLLMFilter && ai.llmSettings && (
          <div style={{ padding: '2px 14px 6px', fontSize: 10, color: 'var(--c-muted)', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      ...
      {parsedTitle && (
        <div style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 14, background: 'var(--bg-subtle)', border: '1px solid var(--bd)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: 'var(--c-muted)', fontWeight: 600 }}>파싱</span>
          <span style={{ fontSize: 13, color: 'var(--c-hint)', flex: 1 }}>{parsedTitle}</span>
          {duration && <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 8, background: 'var(--bg-dim)', color: 'var(--c-faint)' }}>{duration}</span>}
        </div>
      )}
      ...
      {phase >= 3 && (
        <div style={{
          ...
          <span style={{ fontSize: 11, color: 'var(--c-muted)', fontWeight: 600 }}>필터</span>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--c-sub)', userSelect: 'none' }}>
            ...
            <span style={{ minWidth: 28, fontWeight: 700, color: '#1D9E75', fontSize: 12 }}>{minScore}%</span>
          </label>
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 11, color: 'var(--c-muted)', background: 'var(--bg-subtle)', padding: '2px 8px', borderRadius: 10 }}>
      ...
      {points.map((pt, pi) => (
        <div key={pi} style={{ borderRadius: 10, border: '1px solid var(--bd)', background: 'var(--bg-card)', marginBottom: 12, overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--bd)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ width: 22, height: 22, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: pt._isTitlePoint ? '#D85A30' : 'var(--tab-active-bg)', color: 'var(--tab-active-c)', fontSize: 12, fontWeight: 800 }}>{pt._isTitlePoint ? 'T' : pi + 1 - (points[0]?._isTitlePoint ? 1 : 0)}</span>
            <span style={{ fontSize: 13, fontWeight: 700 }}>{pt.title}</span>
            {pt._isTitlePoint && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, background: 'var(--tint-orange)', color: '#D85A30', fontWeight: 600 }}>제목 검색</span>}
          </div>
          <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {(pt.auto_scriptures || []).map((a, i) => (
              <div key={'as' + i} style={{ padding: '8px 10px', borderRadius: 7, background: 'var(--tint-blue)', border: '1px solid var(--tint-blue-bd)' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                  <span style={{ width: 20, height: 20, borderRadius: 4, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff', background: '#D85A30' }}>B</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#2a7ab5' }}>{a.original || a.ref}</span>
                  <span style={{ fontSize: 9, padding: '1px 4px', borderRadius: 4, background: 'var(--tint-green)', color: '#1D9E75', fontWeight: 600 }}>Bible DB</span>
                </div>
                <div style={{ paddingLeft: 28 }}>
                  {a.verses ? a.verses.map((v, vi) => (
                    <div key={vi} style={{ display: 'flex', gap: 8, marginBottom: a.verses.length > 1 ? 3 : 0, fontSize: 13, lineHeight: 1.7 }}>
                      {a.verses.length > 1 && <span style={{ color: '#2a7ab5', fontWeight: 700, minWidth: 18, textAlign: 'right', flexShrink: 0 }}>{v.verse}</span>}
                      <span>{v.text}</span>
                    </div>
                  )) : (
                    <div style={{ fontSize: 13, lineHeight: 1.7 }}>{a.text}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            {(pt.auto_publications || []).map((ap, api) => {
              ...
              return (
                <div key={'ap' + api} style={{ padding: '8px 10px', borderRadius: 7, background: 'var(--tint-purple)', border: '1px solid var(--tint-purple-bd)' }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ width: 20, height: 20, borderRadius: 4, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff', background: '#7F77DD' }}>P</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#6b5fbd' }}>{ap.pub_code}</span>
                    <span style={{ fontSize: 9, padding: '1px 4px', borderRadius: 4, background: 'var(--tint-purple-badge)', color: '#6b5fbd', fontWeight: 600 }}>출판물 DB</span>
                    <div style={{ flex: 1 }} />
                    {!isEditing && (
                      <button onClick={() => setAutoPubEdits(prev => ({ ...prev, [apKey]: { text: currentText, editing: true } }))} style={{
                        padding: '2px 6px', borderRadius: 4, border: '1px solid var(--tint-purple-input)', background: 'var(--bg-card)', color: '#7F77DD', fontSize: 9, cursor: 'pointer', fontWeight: 600,
                      }}>수정</button>
                    )}
                    {isEditing && (
                      <>
                        <button onClick={() => setAutoPubEdits(prev => ({ ...prev, [apKey]: { text: prev[apKey]?.text || bodyText, editing: false } }))} style={{
                          padding: '2px 6px', borderRadius: 4, border: '1px solid #1D9E75', background: 'var(--tint-green)', color: '#1D9E75', fontSize: 9, cursor: 'pointer', fontWeight: 600,
                        }}>확인</button>
                        <button onClick={() => setAutoPubEdits(prev => { const n = { ...prev }; delete n[apKey]; return n; })} style={{
                          padding: '2px 6px', borderRadius: 4, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-muted)', fontSize: 9, cursor: 'pointer',
                        }}>원래대로</button>
                      </>
                    )}
                  </div>
                    ...
                    <div style={{ paddingLeft: 28 }}>
                      <KoreanTextarea value={apEdit.text} onChange={v => setAutoPubEdits(prev => ({ ...prev, [apKey]: { ...prev[apKey], text: v } }))}
                        rows={6} style={{
                          ...
                          fontSize: 13, lineHeight: 1.9, color: 'var(--c-text)', fontFamily: 'inherit', outline: 'none', resize: 'vertical',
                          maxHeight: 200, overflowY: 'auto',
                        }} />
                    </div>
                  ) : (
                    <div style={{ paddingLeft: 28, fontSize: 13, lineHeight: 1.9, color: 'var(--c-text)', whiteSpace: 'pre-wrap', wordBreak: 'keep-all', maxHeight: 132, overflowY: 'auto' }}>
                      {currentText}
                    </div>
                  )}
                </div>
              ...
              return (
                <div key={'sub' + si} style={{ marginLeft: (lvl - 1) * 12, padding: '8px 10px', borderRadius: 7, background: levelBg[lvl] || 'var(--bg-subtle)', border: '1px solid ' + (levelBorder[lvl] || 'var(--bd-soft)') }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 9, padding: '1px 4px', borderRadius: 3, background: c, color: '#fff', fontWeight: 800 }}>{levelLabels[lvl]}</span>
                    <span style={{ fontSize: 12, color: 'var(--c-hint)', fontWeight: 700 }}>{sub.title}</span>
                  </div>
                  {(sub.publications || []).length > 0 && (
                    <div style={{ padding: '4px 8px', marginBottom: 4, borderRadius: 4, background: 'var(--tint-purple)', border: '1px solid var(--tint-purple-bd)', fontSize: 10, color: '#6b5fbd' }}>
                  ...
                  {(sub.auto_scriptures || []).map((a, ai2) => (
                    <div key={'sub-as' + ai2} style={{ padding: '6px 8px', borderRadius: 8, background: 'var(--tint-blue)', border: '1px solid var(--tint-blue-bd)', marginTop: 4 }}>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 3 }}>
                        <span style={{ width: 16, height: 16, borderRadius: 3, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 800, color: '#fff', background: '#D85A30' }}>B</span>
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#2a7ab5' }}>{a.original}</span>
                      </div>
                      <div style={{ paddingLeft: 22 }}>
                        {a.verses.map((v, vi) => (
                          <div key={vi} style={{ display: 'flex', gap: 6, marginBottom: a.verses.length > 1 ? 2 : 0, fontSize: 12, lineHeight: 1.7 }}>
            ...
            <PriorityMaterial
              ...
              onPubAdd={(pubCode) => {
                ...
                setPendingPub({ pub_code: pubCode, point: ap?.point_content || pt.title || '', content, topic: speechTitle || '' });
                setPage('add');
              }}
            />
                  ...
                  {['전체', '표현', '예시', '출판물'].map(f => {
                    ...
                    return (
                      <button key={f} onClick={() => setResultFilter(f)} style={{
                        padding: '2px 8px', borderRadius: 6, border: 'none', fontSize: 9, fontWeight: resultFilter === f ? 700 : 500,
        ...
        <div style={{ borderRadius: 10, border: '1px solid var(--bd)', background: 'var(--bg-card)', padding: 14, marginBottom: 14 }}>
          ...
          <div style={{ borderRadius: 8, border: '1px solid var(--bd-soft)', background: 'var(--bg-card)', marginBottom: 12, overflow: 'hidden' }}>
            <div onClick={() => { setStyleOpen(!styleOpen); if (!styleOpen) { getMyStyles().then(r => setMyStyles(r.styles || [])).catch(() => {}); getPrompts().then(r => { const p = r.prompts || {}; setStylePrompts({ both: p.style_both || '', mine: p.style_mine || '', others: p.style_others || '' }); }).catch(() => {}); } }}
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#7F77DD' }}>연사 스타일 참고</span>
              {(() => { const cnt = myStyles.filter(s => s.selected).length + Object.values(selStyles).filter(Boolean).length; return cnt > 0 ? <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 3, background: '#7F77DD', color: '#fff', fontWeight: 700 }}>{cnt}건</span> : null; })()}
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: 10, color: 'var(--c-dim)' }}>{styleOpen ? '▲' : '▼'}</span>
            </div>
            {styleOpen && (
              <div style={{ padding: 12 }}>
                ...
                <div style={{ marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    <span style={{ fontSize: 11, fontWeight: 600 }}>⭐ 내 스타일</span>
                    <div style={{ flex: 1 }} />
                    <button onClick={() => { setAddingStyle(true); setNewStyleName(''); setNewStyleContent(''); }} style={{ padding: '2px 8px', borderRadius: 4, border: '1px solid #7F77DD', background: 'transparent', color: '#7F77DD', fontSize: 9, cursor: 'pointer', fontWeight: 600 }}>+ 추가</button>
                  </div>
                  {myStyles.map((st, si) => (
                    <div key={si} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid var(--bd-soft)', marginBottom: 4, background: st.selected ? '#7F77DD08' : 'var(--bg-subtle)', fontSize: 10 }}>
                      {editingStyleIdx === si ? (
                        <div>
                          <input value={st.name} onChange={e => { const n = [...myStyles]; n[si] = { ...n[si], name: e.target.value }; setMyStyles(n); }} placeholder="이름" style={{ width: '100%', padding: '3px 6px', borderRadius: 4, border: '1px solid var(--bd)', fontSize: 10, outline: 'none', marginBottom: 4, boxSizing: 'border-box' }} />
                          <textarea value={st.content} onChange={e => { const n = [...myStyles]; n[si] = { ...n[si], content: e.target.value }; setMyStyles(n); }} rows={3} placeholder="스타일 내용"
                            style={{ width: '100%', padding: 6, borderRadius: 6, border: '1px solid var(--bd)', fontSize: 11, fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box', background: 'var(--bg-card)', color: 'var(--c-text-dark)' }} />
                          <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                            <button onClick={() => { saveMyStyles({ styles: myStyles }); setEditingStyleIdx(-1); }} style={{ padding: '3px 10px', borderRadius: 4, border: 'none', background: '#7F77DD', color: '#fff', fontSize: 9, fontWeight: 600, cursor: 'pointer' }}>저장</button>
                            <button onClick={() => setEditingStyleIdx(-1)} style={{ padding: '3px 10px', borderRadius: 4, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-dim)', fontSize: 9, cursor: 'pointer' }}>취소</button>
                          </div>
                        </div>
                      ) : (
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                          ...
                          <button onClick={() => setEditingStyleIdx(si)} style={{ padding: '1px 6px', borderRadius: 3, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-dim)', fontSize: 8, cursor: 'pointer', flexShrink: 0 }}>편집</button>
                          <button onClick={() => { if (!confirm(`"${st.name}" 삭제?`)) return; const n = myStyles.filter((_, i) => i !== si); setMyStyles(n); saveMyStyles({ styles: n }); }} style={{ padding: '1px 6px', borderRadius: 3, border: '1px solid #c44', background: 'transparent', color: '#c44', fontSize: 8, cursor: 'pointer', flexShrink: 0 }}>삭제</button>
                        </div>
                      )}
                    </div>
                  ))}
                  {addingStyle && (
                    <div style={{ padding: '6px 8px', borderRadius: 6, border: '1px dashed #7F77DD', marginBottom: 4, background: 'var(--bg-subtle)' }}>
                      <input value={newStyleName} onChange={e => setNewStyleName(e.target.value)} placeholder="스타일 이름 (예: 도입, 마무리, 예시...)" style={{ width: '100%', padding: '3px 6px', borderRadius: 4, border: '1px solid var(--bd)', fontSize: 10, outline: 'none', marginBottom: 4, boxSizing: 'border-box' }} />
                      <textarea value={newStyleContent} onChange={e => setNewStyleContent(e.target.value)} rows={3} placeholder="스타일 내용 (예: 비유를 사용해줘, 감동적으로...)"
                        style={{ width: '100%', padding: 6, borderRadius: 6, border: '1px solid var(--bd)', fontSize: 11, fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box', background: 'var(--bg-card)', color: 'var(--c-text-dark)' }} />
                      <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                        <button onClick={() => { if (!newStyleName.trim() || !newStyleContent.trim()) return; const n = [...myStyles, { name: newStyleName, content: newStyleContent, selected: true }]; setMyStyles(n); saveMyStyles({ styles: n }); setAddingStyle(false); }} style={{ padding: '3px 10px', borderRadius: 4, border: 'none', background: '#7F77DD', color: '#fff', fontSize: 9, fontWeight: 600, cursor: 'pointer' }}>추가</button>
                        <button onClick={() => setAddingStyle(false)} style={{ padding: '3px 10px', borderRadius: 4, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-dim)', fontSize: 9, cursor: 'pointer' }}>취소</button>
                      </div>
                    </div>
                  )}
                  {myStyles.length === 0 && !addingStyle && <div style={{ fontSize: 10, color: 'var(--c-dim)', padding: '4px 0' }}>[+ 추가]를 눌러 스타일을 등록하세요</div>}
                </div>
                {/* 스타일 검색 */}
                <div style={{ marginBottom: 6 }}>
                  <div style={{ fontSize: 10, color: 'var(--c-dim)', marginBottom: 4 }}>── 스타일 검색 ──</div>
                  <div style={{
                    ...
                    {['전체', '도입', '구조', '성구', '예시', '언어습관', '마무리'].map(cat => (
                      <button key={cat} onClick={() => {
                        ...
                        padding: '4px 10px', borderRadius: 6, fontSize: 10, fontWeight: styleQuery === (cat === '전체' ? '' : cat) ? 700 : 500,
                  ...
                  <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
                    <input value={styleQuery} onChange={e => setStyleQuery(e.target.value)} placeholder="자유 검색 (비유, 감정 묘사...)"
                      ...
                      style={{ flex: 1, padding: '5px 10px', borderRadius: 6, border: 'none', fontSize: 10, background: 'var(--bg-subtle)', outline: 'none', fontFamily: 'inherit', color: 'var(--c-text-dark)' }} />
                    <button onClick={() => { if (!styleQuery.trim()) return; setStyleLoading(true); searchSpeakerMemo({ query: styleQuery, top_k: 20 }).then(r => setStyleResults(r.results || [])).catch(() => {}).finally(() => setStyleLoading(false)); }}
                      disabled={styleLoading} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: styleLoading ? 'var(--bd-medium)' : '#7F77DD', color: '#fff', fontSize: 10, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>{styleLoading ? '...' : '🔍'}</button>
                  </div>
                </div>
                {styleResults.length > 0 && (
                  <div style={{ maxHeight: 240, overflowY: 'auto' }}>
                    {styleResults.map((r, i) => {
                      ...
                      return (
                        <div key={i} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid var(--bd-soft)', marginBottom: 4, background: 'var(--bg-card)', fontSize: 10 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
                            {rating > 0 && <span style={{ fontSize: 9, color: '#C7842D', fontWeight: 700 }}>⭐{rating}</span>}
                            ...
                            {m.memo_category && <span style={{ fontSize: 8, padding: '1px 4px', borderRadius: 3, background: '#7F77DD', color: '#fff' }}>{m.memo_category}</span>}
                            <div style={{ flex: 1 }} />
                            <button onClick={() => {
                            ...
                            }} style={{ padding: '1px 6px', borderRadius: 3, border: '1px solid #7F77DD', background: 'transparent', color: '#7F77DD', fontSize: 8, cursor: 'pointer', fontWeight: 600, flexShrink: 0 }}>+ 추가</button>
                          </div>
                          ...
                          {body.length > 120 && (
                            <button onClick={() => setSelStyles(p => ({ ...p, [`exp_${i}`]: !p[`exp_${i}`] }))} style={{ marginTop: 2, padding: '1px 6px', borderRadius: 3, border: '1px solid var(--bd)', background: 'var(--bg-card)', color: 'var(--c-dim)', fontSize: 8, cursor: 'pointer' }}>{selStyles[`exp_${i}`] ? '접기' : '전체 보기'}</button>
                          )}
                        </div>
                      );
                    })}
                ...
                {styleResults.length === 0 && !styleLoading && styleQuery && (
                  <div style={{ fontSize: 10, color: 'var(--c-dim)', textAlign: 'center', padding: 8 }}>검색 결과가 없습니다</div>
                )}
              </div>
            )}
          </div>
          ...
          <div style={{ borderRadius: 8, border: '1px solid var(--opt-bd)', background: 'var(--opt-bg)', padding: 12, marginBottom: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: '#D85A30', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ fontSize: 12 }}>⚙</span> AI 생성 옵션
            </div>
            <div style={{ marginBottom: 10 }}>
          ...
          {error && (
            <div style={{ padding: '8px 14px', borderRadius: 8, background: 'var(--tint-red)', border: '1px solid var(--tint-red-bd)', color: '#c44', fontSize: 12, marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{error}</span>
              <button onClick={() => setError('')} style={{ border: 'none', background: 'none', color: '#c44', fontSize: 16, cursor: 'pointer', padding: '0 4px', lineHeight: 1, flexShrink: 0 }}>×</button>
            </div>
          )}

          <div style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--bg-subtle)', marginBottom: 12, fontSize: 12, color: 'var(--c-sub)', lineHeight: 1.8 }}>
          ...
          <div style={{
            ...
            <span style={{ padding: '0 10px', fontSize: 14, color: password ? '#1D9E75' : 'var(--c-dim)', flexShrink: 0 }}>🔒</span>
            <input type={showPw ? 'text' : 'password'} placeholder="비밀번호" autoComplete="off" value={password} onChange={e => setPassword(e.target.value)}
              style={{ flex: 1, padding: '9px 0', border: 'none', fontSize: 13, outline: 'none', fontFamily: 'inherit', background: 'transparent', color: 'var(--c-text-dark)', minWidth: 0 }} />
            {password && (
              <button onClick={() => setShowPw(!showPw)} style={{
                padding: '6px 12px', border: 'none', background: 'transparent',
                color: 'var(--c-dim)', fontSize: 10, cursor: 'pointer', fontWeight: 600, flexShrink: 0,
              }}>{showPw ? '숨김' : '표시'}</button>
            )}
          </div>
