...
export async function getPublicationsByGolza(golzaNum) {
  const res = await fetch(`${API}/publications/golza/${encodeURIComponent(golzaNum)}`);
  return res.json();
}
...
export async function outlineDetail(outlineId) {
  const res = await fetch(`${API}/outline/${encodeURIComponent(outlineId)}`);
  return res.json();
}
