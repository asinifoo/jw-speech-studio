...
from services.outline_parser import parse_outline_text, _outline_prefix, _ver_safe, _TYPE_NAMES
...
def save_publication(req: dict):
    ...
    for item in files:
        ...
        for pub in pubs:
            ...
            # 문서 ID: pub_코드_참조
            ref_safe = pub_ref.replace(" ", "").replace("/", "-")[:30] if pub_ref else str(pub_idx).zfill(2)
        ...
        results.append({"outline_num": on, "saved": saved, "updated": updated, "errors": errors})

    _bm25_cache.clear()
    total_new = sum(r.get("saved", 0) for r in results)
    total_upd = sum(r.get("updated", 0) for r in results)
    parts = []
    if total_new: parts.append(f"출판물 {total_new}개 신규")
    if total_upd: parts.append(f"{total_upd}개 기존 (참조 추가)")
    if not parts: parts.append("변경 없음")
    return {"results": results, "total_saved": total_new, "total_updated": total_upd, "message": " · ".join(parts)}
...
def delete_outline(outline_id: str):
    ...
    # JSON 파일 삭제 (버전 포함 파일명 매칭: S-31_001.json, S-31_001_v8-19.json 등)
    import glob
    json_deleted = 0
