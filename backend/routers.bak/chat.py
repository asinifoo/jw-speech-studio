...
def search_points(req: SearchRequest):
        ...
        if pub_all and pub_all["ids"]:
            for pi2, pid in enumerate(pub_all["ids"]):
                ...
                pub_list.append({
                    ...
                    "linked_outlines": meta.get("linked_outlines", ""),
    ...
    for point in req.points:
        ...
        # 검색 결과에 관련 출판물 첨부 (point_num + linked_outlines 매칭)
        if pub_list:
            for item in point_result["search_results"]:
                item_meta = item.get("metadata", {})
                sp_pn = item_meta.get("point_num", "") or item_meta.get("point_id", "")
                sp_on = item_meta.get("outline_num", "") or item_meta.get("golza_num", "")
                if not sp_pn and not sp_on:
                    continue
                matched = []
                for pub in pub_list:
                    linked = pub.get("linked_outlines", "")
                    pub_pid = pub.get("point_id", "")
                    # linked_outlines에 outline_num:point_num 패턴 매칭
                    if sp_on and sp_pn and f":{sp_pn}" in linked:
                        matched.append(pub)
                    elif sp_on and sp_pn and f":{sp_on}-{sp_pn}" in linked:
                        matched.append(pub)
                    # 레거시: point_id 직접 매칭
                    elif pub_pid and sp_pn and (sp_pn == pub_pid or pub_pid.startswith(sp_pn + ".") or sp_pn.startswith(pub_pid + ".")):
                        matched.append(pub)
                if matched:
        ...
        if pub_all and pub_all["ids"]:
            for pi2, pid in enumerate(pub_all["ids"]):
                ...
                pub_list.append({
                    ...
                    "linked_outlines": m.get("linked_outlines", ""),
