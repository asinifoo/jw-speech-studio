...
from services.outline_parser import _outline_prefix, _ver_safe
...
def lookup_pub_title(code: str = ""):
    """출판물 코드로 pub_title + reference 자동 조회"""
    if not code.strip():
        return {"pub_title": "", "pub_type": "", "reference": "", "pub_code": ""}

    # 1. 약어 기반 자동 파싱
    title, ptype, ref, clean_code = _resolve_pub_code(code.strip())
    if title:
        return {"pub_title": title, "pub_type": ptype, "reference": ref, "pub_code": clean_code, "source": "abbreviation"}
    ...
    try:
        ...
        if all_data and all_data["ids"]:
            ...
            for meta in (all_data["metadatas"] or []):
                ...
                if q == pc or q == pa or (q in pc and len(q) > 3):
                    t = meta.get("pub_title", "")
                    if t and t != pc and t != pa:
                        return {"pub_title": t, "pub_type": meta.get("pub_type", ""), "reference": meta.get("reference", ""), "pub_code": pc, "source": "db"}
    except Exception:
        pass
    # 면/항만 분리해서 반환
    _, _, ref, clean_code = _resolve_pub_code(code.strip())
    return {"pub_title": "", "pub_type": "", "reference": ref, "pub_code": clean_code}
...
def outline_detail(outline_id: str, outline_type: str = ""):
...
def db_add(req: DbAddRequest):
    ...
    if req.entry_type == "publication":
        ...
        pub_code = (formatted_pub_code + " " + ref).strip() if ref else formatted_pub_code
